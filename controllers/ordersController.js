...
const addOrder = async (req, res) => {
    ...
    try {
        ...
        const customerResult = await pool.query('SELECT id FROM YuNowDataBase.customers WHERE id = $1', [customer_id]);
        if (customerResult.rows.length === 0) {
            return res.status(404).json({ error: 'Cliente no encontrado' });
        }

        const addressResult = await pool.query('SELECT id FROM YuNowDataBase.addresses WHERE id = $1', [address_id]);
...
const getOrders = async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM YuNowDataBase.orders');
...
const getOrderById = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('SELECT * FROM YuNowDataBase.orders WHERE id = $1', [id]);
...
const getOrderByCustomerId = async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM YuNowDataBase.orders WHERE customer_id = $1', [customer_id]);
        if (result.rows.length === 0) {
...
const deleteOrder = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('DELETE FROM YuNowDataBase.orders WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Orden no encontrada' });
        }
...
const updateOrderPartial = async (req, res) => {
    ...
    try {
        const existingOrder = await pool.query('SELECT * FROM YuNowDataBase.orders WHERE id = $1', [id]);
